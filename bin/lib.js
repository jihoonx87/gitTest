...
TabKeyController.prototype.focusOnInit = function(flag)
{
	if(flag)
	{
		...
		else
		{
			...
			{	
				...
				AComponent.setFocusComp(this.rootView);
			}
		}
	}
};
...
AContainer.prototype.open = async function(url, parent, left, top, width, height)
{
	...
	//비동기 구조로 인해 _after_setview 로 이동
	...
	function _after_setview()
	{
        ...
        //무조건 나중에 추가해야 함.. 대신 thisObj.onResize(); 가 호출되도록 함.
		_append_helper();

		thisObj.tabKey.focusOnInit(thisObj.option.focusOnInit);
                ...
                timer = setTimeout(async function() 
                {
                    ...
                    for(let i=0; i<info.views.length; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if(cntr) cntr.setView(cntr.getView().url)
                        }
