			...
			if(hCellArr.length == 1 || bCellArr.length == 1 || fCellArr.length == 1)
			{
				...
				switch (type)
				{
					...
					case '101':	//head foot
						...
						setCell(hCellArr[0][0], this.showThead.children().length-1, this.getHeaderCell);
...
AGrid.prototype.showColumn = function(colIdx)
{
	...
	if(this.option.isColumnResize && this.resizeBars[colIdx])
	{
		...
		this.resizeBars[colIdx].css('left', $td.position().left + $td.outerWidth() + 'px');
...
AGrid.prototype.updatePosition = function(pWidth, pHeight)
{
	...
	if(!this.option.isHideHeader && this.option.isColumnResize) this._updateBarPos();
};
...
AGrid.prototype.columnResizable = function(callback)
{
	...
	var thisObj = this, pos, idx, hRowCount = this.getHeaderRowCount(), isLastColumn = false,
		endIdx = thisObj.$headCols.length/hRowCount - 1,
	...
	this.$headCols.each(function(i)
	{
		...
		pos = $(this).position().left + $(this).outerWidth() - 2;
		...
		thisObj._resizeProcess($sizeBar, idx);
		
		arr[idx] = true;
	});
	
	//리사이즈를 활성화 시키면 마지막 컬럼에 셋팅된 값을 지워야 정상 작동한다.
	this.setColumnWidth(colLen-1, '');
...
AGrid.prototype._resizeProcess = function($sizeBar, inx)
{
	...
	{ 
		axis: 'x',
		containment: this.option.isWidthChangable?"#thumbnail":"parent",
		cursor: "e-resize",
		//helper: "clone",
...
AGrid.prototype._colculResizeWidth = function(inx, newLeft)
{
	...
	if((!val || val.indexOf('%')>-1) && !this.option.isWidthChangable)
	{
	...
	else
	{
		...
		{
			changedWidth = this.getWidth() + newLeft - this.resizeBars[inx][0].beforeLeft;
			this.setWidth(changedWidth);
		}
	}
	if(this.resizeCallback) this.resizeCallback(changedWidth);
};
