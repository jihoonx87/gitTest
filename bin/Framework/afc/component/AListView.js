...
AListView.prototype.insertItem = async function(url, dataArray, posItem, isPrepend)//, asyncCallback)
{
	var newItems = null, thisObj = this;
	
	newItems = await this.createItems(url, dataArray, posItem, isPrepend);
...
AListView.prototype.addItem = async function(url, dataArray, isPrepend)//, asyncCallback)
{
	var newItems = await this.insertItem(url, dataArray, null, isPrepend);//, asyncCallback);
	
	return newItems;
};

//여러개의 url 을 동시에 추가한다. 단, urlArr 과 dataArr 은 1:1 로 매칭된다.
AListView.prototype.addItems = async function(urlArr, dataArr, isPrepend)//, asyncCallback)
{
...
AListView.prototype.createItems = async function(url, dataArray, posItem, isPrepend)//, asyncCallback)
{
