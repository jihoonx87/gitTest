...
ADataGrid.prototype._onScrollY = function(acomp, info, e)
{
	...
	if(newInx != this.dataInx) 
	{
        ...
        //아래 코드로 성능 저하가 발생한다. 필요없는 경우 안 할 수 있도록 살펴보기
		this._resetInitRow(true);
...
/*
ADataGrid.prototype._gridInputClick = function(e)
{
...
ADataGrid.prototype._maskRowData = function(rowData)
{
    ...
    for(i=0; i<rowData.length; i++)
    {
        ...
        if(cell.dm) 
        {
            ...
            let retObj = cell.dm.mask(dataObj.text, null, dataObj)
            if(typeof(retObj)=='string') dataObj.text = retObj
            else rowData[i] = retObj
        }
    }
...
ADataGrid.prototype._maskCellData = function(colInx, cellData)
{
    ...
    if(cell.dm) 
    {
        ...
        if(typeof(retObj)=='string') cellData.text = retObj;
        else cellData = retObj;
    }
...
{
	this.dataArr2[rowInx][colInx] = this._maskCellData(rowInx, colInx, cellData);
...
ADataGrid.prototype.mergeCellData = function(rowInx, colInx, cellData, noUpdate)
{
	...
	afc.mergeObject(curData, this._maskCellData(rowInx, colInx, cellData));
...
{
	let $cells = $(grid.getRow(rowInx)).children(), text;
	...
	let i, obj, cell, checked = '', childEle;
	for(i=start; i<=end; i++)
	{
		...
		if(obj.type)
		{
            ...
            else
            {
                ...
                cell.innerHTML = `<input type="${obj.type}" value="${obj.value}" ${checked} />`
